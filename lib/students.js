...
const BASE_COLUMNS = "id, display_name, birth_year, created_at";
const LOGIN_COLUMNS = "auth_user_id, login_email";
...
export async function loadStudent(supabase, therapistId, studentId) {
  ...
  const { data, error } = await read(`${BASE_COLUMNS}, ${LOGIN_COLUMNS}`);
  if (!error) return data ?? null;

  // Exactly one error is worth a second attempt: supabase/student_accounts.sql
  // has not been applied, so the login columns do not exist and PostgREST
  // rejects the ENTIRE select — one absent column loses the whole row.
  //
  // Reading only `data` here used to swallow that error and return null, which
  // the student page turned into a 404 for a child whose profile, screenings
  // and journey were all present. None of that needs the login columns, so the
  // therapist's page has no business breaking over them. loadStudentSummaries
  // below already degrades rather than fails; this is the same contract.
  if (!isSchemaMissing(error)) return null;

  const { data: base } = await read(BASE_COLUMNS);
  if (!base) return null;

  // Flagged, not merely null: null reads as "no login issued yet", and the panel
  // would offer a form whose write cannot land — creating an auth user that
  // nothing points at.
  return { ...base, auth_user_id: null, login_email: null, loginsUnavailable: true };
}
