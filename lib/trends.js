...
function toPoint(row) {
  ...
  const rawErrorRate = hasWords && hasErrors ? (errors / words) * 100 : null;
  const roundedErrorRate = Number.isFinite(rawErrorRate) ? round(rawErrorRate) : null;
  const errorsPer100 = Number.isFinite(roundedErrorRate) ? roundedErrorRate : null;
