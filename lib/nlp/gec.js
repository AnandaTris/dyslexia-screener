...
export async function correctSentences(sentences, { timeoutMs = 120_000 } = {}) {
  ...
  // The timer has to be cleared on the winning path too. Left pending it keeps
  // the event loop alive for the full budget, which turns a 1.4s analysis into
  // a 120s hang in `npm run warm:nlp` and into 120s of billed time serverless.
  let loadTimer = null;
  const corrector = await Promise.race([
    loadCorrector(),
    new Promise((resolve) => {
      loadTimer = setTimeout(() => resolve("timeout"), timeoutMs);
    }),
