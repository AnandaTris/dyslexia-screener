...
describe("the screener → analyser handover", () => {
  it("hands the sample over once, verbatim, and then erases it", () => {
    ...
    // studentId comes back null rather than absent: the screener may not have
    // supplied one, and the analyser distinguishes "nobody was pre-selected"
    // from a missing field.
    expect(takeHandoff()).toEqual({ text: transcription, writerAge: 8, studentId: null });
  ...
  it("carries the screened student across, so they are not picked twice", () => {
    // Both pages now file their result against a student. Without this the
    // educator re-picks on the analyser page, and a mis-pick files one child's
    // errors onto another child's trend line.
    stashHandoff({ text: "the kat sat", writerAge: 8, studentId: "student-7" });

    expect(takeHandoff()).toEqual({
      text: "the kat sat",
      writerAge: 8,
      studentId: "student-7",
    });
  ...
  it("pre-selects nobody when the stored student id is stale or the wrong type", () => {
    // An entry written by an older build has no studentId at all, and a
    // hand-edited one can hold anything. Either way the picker should open
    // empty rather than resolve to a student that is not there.
    seedRaw(JSON.stringify({ text: "the kat sat", writerAge: 8 }));
    expect(takeHandoff()?.studentId).toBeNull();

    seedRaw(JSON.stringify({ text: "the kat sat", writerAge: 8, studentId: 42 }));
    expect(takeHandoff()?.studentId).toBeNull();
  });
