 ...
 *
 * Returns the verdict, the cleaned score, and a `reason` that is non-null only
 * when the rule overrode a score that would otherwise have read "likely". The
 * reason has to be shown: without it an educator sees "unlikely" above a gauge
 * reading 70 and has no way to understand why.
 */
export function decideVerdict({
  ...
  if (score < LIKELY_SCORE_THRESHOLD) {
    return { verdict: "unlikely", score, reason: null };
  }
  ...
  return { verdict: "likely", score, reason: null };
}
